...
// 🔗 source: https://raw.githubusercontent.com/fsamin/go-dump/c877b85e7fc1e8c59fad34c8f986e786ea76dc01/dump_test.go
// 📝 license: Apache-2.0
...
func TestRecursiveEnumDump(t *testing.T) {
	// Create test structures that will cause infinite recursion
	enum := &EnumModel{
		Name:        "TestEnum",
		BaseType:    "string",
	...
	value := EnumValue{
		...
		Parent:      enum, // This creates the circular reference
	}
	...
	// Run the dump in a goroutine with a timeout
	go func() {
		out := &bytes.Buffer{}
		err := dump.Fdump(out, enum)
		if err != nil {
			t.Logf("Expected timeout, but got error: %v", err)
		}
		done <- true
	}()
	...
	select {
	case <-done:
		t.Error("Test should have timed out due to infinite recursion, but it completed")
	case <-time.After(1 * time.Second):
		// This is the expected path - we should timeout
		t.Log("✅ Test correctly timed out due to infinite recursion")
	}
}
